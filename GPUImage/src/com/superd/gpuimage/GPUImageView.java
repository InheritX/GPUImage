...
//		}
		...
		mSizeInPixels = new AndroidSize(getWidth(), getHeight());
	}
...
//		});
	...
	private void recalculateViewGeometry() {
		mImageVertices = AndroidUtils.directFloatBufferFromFloatArray(AndroidConstants.imageVertices);
	}
	...
	public void newFrameReadyAtTime(long frameTime, int textureIndex) {
		GPUImageOutput.runSynchronouslyOnVideoProcessingQueue(new Runnable(){
			...
			public void run() {
				...
				GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mInputFramebufferForDisplay.getmTexture());
	...
	public void setInputFramebuffer(GPUImageFramebuffer newInputFramebuffer,
		...
		mInputFramebufferForDisplay.lock();
	}
	...
	@Override
	public void setInputSize(final AndroidSize newSize, int index) {
		GPUImageOutput.runSynchronouslyOnVideoProcessingQueue(new Runnable(){
			@Override
			public void run() {
	...
	private AndroidSize mSizeInPixels = null;
    ...
    private AndroidSize mBoundsSizeAtFrameBufferEpoch = null;
    private SurfaceHolder.Callback mHolderCallback = null;
    private AndroidSize mInputImageSize = null;
